                        ...
                        Method(
                          (b) =>
                              b
                                ...
                                ..name =
                                    '${api.nameWithoutGroup.lowerCamel}Nullable'
                                ..annotations.add(
                                  _jsAnnotation(
                                    api.nameWithoutGroup.lowerCamel,
                                  ),
                                )
...
'''),
                ...
                TypeDef(
                  (b) =>
                      b
                        ..name = type.name
                        ..docs.add(
                          documentationComment(type.documentation, indent: 0),
                        )
              ...
              for (var type in api.dictionaries.where(
                (d) => !d.isSyntheticEvent,
              ))
                ..._dictionary(type),
            ]),
    );
              ...
              (p) => Parameter(
                (b) =>
                    b
                      ...
                      ..type = p.type.jsType
                      ..docs.add(
                        documentationComment(p.documentation, indent: 4),
                      ),
              ),
            )
            .toList();
            ...
            (b) =>
                b
                  ...
                  ..optionalParameters.addAll(
                    type.properties.map(_typePropertyAsParameter),
                  ),
          ),
        )
...
class DartApiGenerator extends _GeneratorBase {
      ...
      (b) =>
          b
            ...
            ..directives.addAll([
              Directive.export(
                'src/chrome.dart',
                show: ['chrome', 'EventStream'],
              ),
              Directive.import(_internalHelpers),
              if (api.group case var group?)
                Directive.export(
                  '${group.snakeCase}.dart',
                  show: [
              ...
              Field(
                (b) =>
                    b
                      ...
                      ..assignment =
                          refer(mainClassName).property('_').call([]).code,
              ),
              Extension(
                (b) =>
                    b
              ...
              Class(
                (b) =>
                    b
                      ...
                      ..methods.addAll(
                        api.functions.map(
                          (f) => _function(f, source: _referJsBinding()),
                        ),
                      )
                      ..methods.addAll(api.properties.map(_property))
                      ..methods.addAll(
                        api.events.map(
                          (e) => _event(e, source: _referJsBinding()),
                        ),
                      ),
              ),
    ...
    return Method(
      (b) =>
          b
            ...
            ..returns = refer('bool')
            ..body =
                referTo.notEqualTo(refer('null')).and(refer('alwaysTrue')).code,
    );
  }
  ...
  Method _function(model.Method method, {required Expression source}) {
    ...
    if (method.returns.type case model.AsyncReturnType asyncReturn) {
      ...
      if (asyncReturn.supportsPromises) {
        ...
        body = _asyncReturnCodeWithPromise(
          asyncReturn,
          referTo: referTo,
        ...
        body = _asyncReturnCodeWithCallback(
          asyncReturn,
          referTo: referTo,
          callParameters: callParameters,
          futureType: futureType,
      ...
      body = Block.of([callExpression.statement]);
    ...
    var requiredParameters = method.parameters.map(
      (p) => Parameter(
        (b) =>
            b
              ..name = p.dartName
          ...
          b
            ...
            ..docs.addAll([
              ...
              if (method.returns.documentation case var doc?)
                if (doc.isNotEmpty)
                  parameterDocumentation('returns', doc, indent: 4),
            ])
            ..annotations.addAll(_deprecatedAnnotation(method.deprecated))
            ..name = method.name
            ..returns = returns
  ...
  Block _asyncReturnCodeWithCallback(
    AsyncReturnType asyncReturn, {
    ...
    var completeCompleterCode = refer(
      completerVar,
    ).property('complete').call([completeParameter]).accept(emitter);

    var body = Block.of([
      declareVar(
        completerVar,
      ).assign(refer('Completer').call([], {}, [futureType])).statement,
      referTo.call([
        ...callParameters,
        Method(
          (b) =>
              b
                ...
                ..requiredParameters.addAll([
                  for (var jsParam
                      in asyncReturn.jsCallback.positionalParameters)
                    Parameter(
                      (b) =>
                          b
  ...
  Code _asyncReturnCodeWithPromise(
    AsyncReturnType asyncReturn, {
    required Reference futureType,
    required Expression referTo,
    required List<Expression> callParameters,
    ...
    return Enum(
      (b) =>
          b
            ...
            ..docs.add(
              documentationComment(enumeration.documentation, indent: 0),
            )
            ..fields.add(
              Field(
                (b) =>
...
'''),
  ...
  Iterable<Spec> _dictionary(model.Dictionary dictionary) sync* {
    var type = model.DictionaryType(
      dictionary.name,
      locationFile: api.fileName,
      isNullable: false,
    );
            ...
            Method(
              (b) =>
                  b
                    ...
                    ..body = Code(
                      'super.$wrappedVariable as $_jsPrefix.${dictionary.name}',
                    )
                    ..lambda = true
            ...
            Constructor(
              (b) =>
                  b
                    ...
                    ..initializers.add(
                      refer(wrappedVariable).assign(constructorSetter).code,
                    ),
            ),
          );
          b.methods.add(
        ...
        b
          ...
          ..methods.addAll(
            dictionary.methods.map(
              (f) => _function(f, source: refer(wrappedVariable)),
            ),
          )
          ..methods.addAll(
            dictionary.events.map(
              (e) => _event(e, source: refer(wrappedVariable)),
            ),
          );
      }
    });
...
class _PrefixedAllocator implements Allocator {
  ...
  Iterable<Directive> get imports =>
      _imports.keys.map((u) => Directive.import(u, as: _imports[u]));
}
            ...
            Field(
              (b) =>
                  b
                    ...
                    ..modifier = FieldModifier.final$
                    ..assignment =
                        refer(groupClass).property('_').call([]).code,
            ),
            Extension(
              (b) =>
                  b
